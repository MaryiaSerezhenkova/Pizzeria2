...
public class JsonConverter {
	...
	public static String fromMenuRowToJson(IMenuRow iMenuRow) throws Exception {
		try {
			return new ObjectMapper().registerModule(new JavaTimeModule()).writeValueAsString(iMenuRow);
		} catch (JsonProcessingException e) {
			throw new Exception("Failed to write MenuRow:" + iMenuRow.toString() + "as json", e);
		}
	}

	public static MenuRowDTO fromJsonToMenuRow(String jsonString) throws JsonProcessingException {
		return new ObjectMapper().readValue(jsonString, MenuRowDTO.class);
	}

	public static String fromMenuRowListToJson(List<IMenuRow> list) throws Exception {
		try {
			return new ObjectMapper().registerModule(new JavaTimeModule()).writeValueAsString(list);
		} catch (JsonProcessingException e) {
			throw new Exception("failed to write List MenuRow as json", e);
		}
	}
